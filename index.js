...
bot.on("message", msg => {
  ...
  if ((msg.content.toLowerCase().includes("where") && msg.content.toLowerCase().includes("nude")) || (msg.content.toLowerCase().includes("i") && msg.content.toLowerCase().includes("nude"))) {
    let has = false
    rankroles.forEach(r => {
      if (msg.member.roles.cache.get(r)) has = true
    })
    if (has) msg.reply("It seems like you are looking for NSFW(nude) channels, I suggest you to check out the <#721635790760706079> channel for more info.")
  }
  //Attachment limiter
